...
from ReadData import GetBatch
from ReadData import Image
...
#   
#    #cv2.imwrite("yeni_resimler/yeni_resim"+str(t)+".jpg", resized)
#    cv2.waitKey(0)
...
if __name__=="__main__":
    ...
    new_list=GetBatch(0,3)
    print(len(new_list))
    for i in range(len(new_list)):        
        """resize with OpenCv ."""        
        img = cv2.imread('./val2017img/val2017/'+Image.__str__(new_list[i]), cv2.IMREAD_UNCHANGED)
        print(img)
        print("*********************************************jhvkbxklcjb<l")
